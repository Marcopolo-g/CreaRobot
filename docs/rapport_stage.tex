...
%   3. gpt-image-1 (images.edit) utilise la description + l'image originale
...
%
% Cold start gpt-image-1 : le premier appel d'une session peut atteindre ~63 s
...
% Un transcript STT résiduel ("fini", prononcé en fin de phase DRAWING) peut
% arriver dans handle_dialogue après la transition vers START_FEEDBACK, avant
% que generate_c1_feedback() n'ait produit sa première réponse. Sans garde,
% ce transcript est traité comme un échange valide et incrémente dialogue_count,
% rendant le nombre d'échanges inconsistant entre les tours de feedback.
...
%   gpt-image-1 génération      2   20.12 s  63.94 s  42.03 s
%   C2 total                    2   24.88 s  71.33 s  ~48 s
%
% Observations :
% - La variance de gpt-image-1 est très élevée (×3 entre min et max) ;
